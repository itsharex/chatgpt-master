  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="角色名称" prop="title">
        <el-input v-model="queryParams.title" placeholder="请输入角色名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['gpt:assistant:save']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['gpt:assistant:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['gpt:assistant:remove']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="assistantList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="角色名称" align="center">
        <template slot-scope="scope">
          {{ parseEmoji(scope.row.icon) + ' ' + scope.row.title }}
        </template>
      </el-table-column>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="角色图标" prop="icon">
          <el-select v-model="form.icon" filterable placeholder="请选择角色图标">
            <el-option v-for="item in emojis" :key="item.name" :label="item.name + item.emoji"
        ...
        <el-form-item label="角色描述" prop="description">
          <el-input v-model="form.description" type="textarea" :autosize="{ minRows: 1, maxRows: 2 }"
            placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="系统提示词" prop="systemPrompt">
          <el-input v-model="form.systemPrompt" type="textarea" :autosize="{ minRows: 2, maxRows: 3 }"
            placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="AI打招呼" prop="firstMessage">
          <el-input v-model="form.firstMessage" type="textarea" :autosize="{ minRows: 2, maxRows: 3 }"
            placeholder="请输入内容" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除AI助理功能编号为"' + ids + '"的数据项？').then(function () {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/gpt/assistant/export', {
        ...this.queryParams
