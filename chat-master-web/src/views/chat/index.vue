...
import { listAssistantType, fetchChatAPI, fetchChatMessageAPI, fetchChatAPIProcess, fetchChatMessageById, deleteChatMessageById } from '@/api'
...
window.addEventListener('resize', () => {
  computedHeight.value = (window.innerHeight - ideaRef.value.clientHeight - 2) + 'px';
});
...
onMounted(() => {
  ...
  computedHeight.value = (window.innerHeight - ideaRef.value.clientHeight - 74) + 'px';
...
function assistantType() {
  listAssistantType<Array<AssistantType>>().then((res: { data: { id: number; name: string; icon: string }[] }) => {
...
async function onConversation(chatNumber: string, conversationId: string) {
  ...
  try {
    ...
    const fetchChatAPIOnce = async () => {
        ...
        onDownloadProgress: ({ event }) => {
          ...
          try {
            ...
            updateChat(chatNumber, dataSources.value.length - 1, {
              ...
              images: data.contentType == 'image' && data.content ? data.content.map((d: { b64Image: any }) => { return d.b64Image }) : [],
              model: chatStore.model,
              inversion: false,
              error: false,
              loading: true,
      ...
      updateChatLoading(chatNumber, dataSources.value.length - 1, { loading: false })
    }
    ...
    const currentChat = getChatByIndex(chatNumber, dataSources.value.length - 1)
  ...
  <div class="flex w-full h-full bg-slate-50 dark:bg-[#18181c]">
    <div class="flex flex-col h-full m-auto" :class="indexClass">
      <main class="w-full pt-4 mb-8 flex-1 overflow-hidden">
        <div id="scrollRef" ref="scrollRef" class="h-full overflow-hidden overflow-y-auto no-scrollbar">
            ...
            <template v-else>
              <div>
                <Message v-for="(item, index) of dataSources" :key="index" :date-time="item.dateTime"
                  :contentType="item.contentType" :text="item.text" :images="item.images" :model="item.model"
                  :inversion="item.inversion" :error="item.error" :loading="item.loading"
                  @regenerate="onRegenerate(item.parentMessageId)" @delete="handleDelete(index, item.conversationId)" />
                <div class="h-10"></div>
      ...
      <div v-if="showPrompt" class="w-[510px] bg-[#fff] dark:bg-[#24272e] pb-0">
        <div ref="ideaRef" class="p-4 pb-2 h-auto">
          ...
          <div class="flex flex-wrap">
            <div class="text-sm px-3 py-1 rounded-md mr-2 mb-2 cursor-pointer  "
              ...
              v-for="item in assistantTypeValue" :key="item.id" @click="chooseType(item.id)">{{ item.name }}</div>
          </div>
        </div>
        <div :style="{ height: computedHeight, maxHeight: computedHeight }">
          <WaterfallFlow :assistantTypeId="assistantTypeId" @handAssistant="handAssistant"
            @editAssistant="editAssistant" />
        </div>
      </div>
      <div v-else class="w-16 h-full flex flex-col items-center justify-center">
        <div class="flex flex-col items-center justify-center w-16 bg-white dark:bg-[#24272e] rounded-l-lg p-3">
          <div @click="chooseFixedType(item.id)"
            class="flex flex-col items-center p-2 rounded-lg cursor-pointer hover:text-blue-600"
            v-for="item in fixedAssistantTypeValue" :key="item.id">
            <SvgIcon :icon="item.icon" class="mb-2 text-xl" />
            <div class="text-xs">{{ item.name }}</div>
          </div>
          <div class="flex flex-col items-center w-14 p-2 rounded-lg cursor-pointer hover:text-blue-600">
            <div class="flex items-center justify-center w-6 h-6 bg-slate-100 dark:bg-[#24272e] rounded-xl"
